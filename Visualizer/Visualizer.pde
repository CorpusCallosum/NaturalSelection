...
void setup() {
  ...
  //load the data file
   _jsonData = loadStrings("../../NaturalSelection/data/data.txt");
}
...
void draw() {
  ...
  //parse data
   try {
    data = new JSONObject(_jsonData[_dCounter]);
  }
 ...
 try {
      ...
      if (eventType.equals("generation")) {
        ...
        JSONArray jsonGenes = data.getJSONArray("momGenes");
        float[] genes = new float[jsonGenes.length()];
        //convert json array to java array
        for (int i=0; i<jsonGenes.length(); i++) {
          genes[i] = (Float)(Double)jsonGenes.get(i)f;
        }

        //set the DNA on the child
        popul.setDNA(genes);
